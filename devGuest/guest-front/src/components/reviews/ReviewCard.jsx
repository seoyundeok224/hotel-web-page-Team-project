...
const ReviewCard = ({ review, isOwner, currentUser, token, onAction, setSnackbar }) => {
    ...
    useEffect(() => {
        ...
        setCommentCount(review.comments?.length || 0);
    }, [review]);
    ...
    const handleLike = async () => { if (!token || isLiking) return; const originalReview = { ...currentReview }; const originalLiked = liked; const newLiked = !originalLiked; const newLikeCount = newLiked ? originalReview.likeCount + 1 : originalReview.likeCount - 1; setLiked(newLiked); setCurrentReview(prev => ({ ...prev, likeCount: newLikeCount })); const store = JSON.parse(localStorage.getItem('likedReviews') || '[]'); if (newLiked) { localStorage.setItem('likedReviews', JSON.stringify([...store, review.id])); } else { localStorage.setItem('likedReviews', JSON.stringify(store.filter(id => id !== review.id))); } setIsLiking(true); try { await reviewService.likeReview(currentReview.id, token); } catch (err) { setSnackbar({ open: true, message: '오류가 발생했습니다. 다시 시도해주세요.', severity: 'error' }); setCurrentReview(originalReview); setLiked(originalLiked); localStorage.setItem('likedReviews', JSON.stringify(store)); } finally { setIsLiking(false); } };
    ...
    return (
        <>
            <Card variant="outlined" sx={{ '&:hover': { boxShadow: 3 } }}>
                <CardHeader avatar={<Avatar sx={{ bgcolor: stringToColor(currentReview.username) }}>{currentReview.username.charAt(0)}</Avatar>} action={isOwner && <IconButton onClick={handleMenuOpen}><MoreVertIcon /></IconButton>} title={currentReview.username} subheader={new Date(currentReview.createdAt).toLocaleString('ko-KR')} />
                {isEditing ? ( <CardContent sx={{ pt: 0, display: 'flex', flexDirection: 'column', gap: 2 }}> <Rating value={editedRating} onChange={(e, newValue) => setEditedRating(newValue)} /> <TextField fullWidth multiline rows={4} value={editedContent} onChange={(e) => setEditedContent(e.target.value)} variant="outlined"/> <Box sx={{ display: 'flex', justifyContent: 'flex-end', gap: 1 }}> <Button onClick={handleCancelEdit}>취소</Button> <Button variant="contained" onClick={handleSaveEdit}>저장</Button> </Box> </CardContent> ) : ( <CardContent sx={{ pt: 0 }}> <Rating value={currentReview.rating} readOnly /> { isLongReview ? ( <> <Collapse in={isExpanded} collapsedSize={100} timeout="auto"> <Typography variant="body2" sx={{ whiteSpace: 'pre-wrap', mt: 1 }}> {currentReview.content} </Typography> </Collapse> <Button size="small" onClick={handleExpandClick} sx={{ mt: 1 }}> {isExpanded ? '간략히' : '...더보기'} </Button> </> ) : ( <Typography variant="body2" sx={{ whiteSpace: 'pre-wrap', mt: 1 }}> {currentReview.content} </Typography> )} </CardContent> )}
