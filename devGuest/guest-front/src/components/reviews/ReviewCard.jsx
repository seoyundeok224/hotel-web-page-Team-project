...
const ReviewCard = ({ review, isOwner, currentUser, token, onAction, setSnackbar }) => {
    ...
    const handleSaveEdit = async () => { try { await reviewService.updateReview(currentReview.id, { content: editedContent, rating: editedRating }, token); onAction(); setSnackbar({ open: true, message: '후기가 수정되었습니다.', severity: 'success' }); setIsEditing(false); } catch (err) { setSnackbar({ open: true, message: '후기 수정에 실패했습니다.', severity: 'error' }); } };
    ...
    const handleLike = async () => {
        ...
        setLiked(newLiked);
        setCurrentReview(prev => ({ ...prev, likeCount: newLiked ? (prev.likeCount || 0) + 1 : (prev.likeCount || 0) - 1 }));
    ...
    const handleCommentChange = (change) => { setCommentCount(prevCount => prevCount + change); };

    return (
        <>
            <Card variant="outlined" sx={{ transition: 'transform 0.2s ease-in-out, box-shadow 0.2s ease-in-out', '&:hover': {  transform: 'translateY(-4px)', boxShadow: 6 } }}>
                <CardHeader 
                    ...
                    title={<Box sx={{display: 'flex', alignItems: 'center'}}><Typography variant="body2" component="strong" sx={{ fontWeight: 'bold' }}>{currentReview.username || '알 수 없는 사용자'}</Typography><Timestamp date={currentReview.createdAt} /></Box>}
                    subheader={<Rating value={currentReview.rating || 0} readOnly size="small" />}
                />
                {isEditing ? ( 
                        ...
                        {isLongReview ? ( 
                            <> 
                                <Collapse in={isExpanded} collapsedSize={100} timeout="auto"> 
                                    <Typography variant="body2" sx={{ whiteSpace: 'pre-wrap', mt: 1 }}>{currentReview.content || ''}</Typography> 
                                </Collapse> 
                                <Button size="small" onClick={handleExpandClick} sx={{ mt: 1 }}>{isExpanded ? '간략히' : '...더보기'}</Button> 
                            </> 
                        ) : ( 
                            <Typography variant="body2" sx={{ whiteSpace: 'pre-wrap', mt: 1 }}>{currentReview.content || ''}</Typography> 
                        )} 
                ...
                <Collapse in={showComments} timeout="auto" unmountOnExit>
                    <CommentSection reviewId={currentReview.id} token={token} currentUser={currentUser} setSnackbar={setSnackbar} onCommentChange={handleCommentChange} />
                </Collapse>
            </Card>
