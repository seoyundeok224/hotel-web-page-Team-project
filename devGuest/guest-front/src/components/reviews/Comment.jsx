...
const Comment = ({ comment, reviewId, token, currentUser, onCommentCreated, setSnackbar }) => {
          ...
          {comment.children?.length > 0 && (
            <Box sx={{ mt: 1, pl: 2, borderLeft: '2px solid #eee' }}>
              {comment.children.map((childComment) => (
                <Comment key={childComment.id} comment={childComment} reviewId={reviewId} token={token} currentUser={currentUser} onCommentCreated={onCommentCreated} setSnackbar={setSnackbar}/>
              ))}
            </Box>
          )}
        </Box>
